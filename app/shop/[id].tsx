  ...
  return (
    <View style={{ width: 140 }} className="bg-navy-800 rounded-2xl overflow-hidden mr-3">
      ...
      <View className="p-3">
        ...
        {servicio.duracion ? (
          <Text className="text-ink-secondary text-xs mt-1">{servicio.duracion}</Text>
        ) : null}
        {servicio.precio != null ? (
          <Text className="text-accent text-xs font-bold mt-1">Bs. {servicio.precio}</Text>
        ) : null}
      </View>
    </View>
  );
  ...
  return (
    <View style={{ width: 140 }} className="bg-navy-800 rounded-2xl overflow-hidden mr-3">
      ...
      <View className="p-3">
        ...
        {precio != null ? (
          <Text className="text-accent text-xs font-bold mt-1">Bs. {precio}</Text>
        ) : null}
      </View>
    </View>
  );
...
export default function PublicShopProfile() {
  ...
  const { data: tallerData, isLoading: loadingTaller, isError: errorTaller } =
    usePublicTallerProfile(entidadTipo === 'taller' ? id : '');
  const { data: proveedorData, isLoading: loadingProveedor, isError: errorProveedor } =
    usePublicProveedorProfile(entidadTipo === 'proveedor' ? id : '');
  ...
  const entityName  = entidadTipo === 'taller'
    ? tallerData?.taller.nombre_taller
    : proveedorData?.proveedor.nombre_proveedor;
  const entityImage = entidadTipo === 'taller'
    ? tallerData?.taller.perfil_taller_url
    : proveedorData?.proveedor.perfil_proveedor_url;

  const { data: currentProfile } = useCurrentProfile();
  const conductorId = currentProfile?.role === 'conductor'
    ? currentProfile.usuario.id_usuario
    : null;

  const handleRuta = (lat: number | null | undefined, lng: number | null | undefined) => {
    if (!lat || !lng || lat === 0 || lng === 0) return;
    const mapRoute = currentProfile?.role === 'conductor' ? '/(conductor)/map' : '/(shop)/map';
    router.push({ pathname: mapRoute as any, params: { focusLat: lat.toString(), focusLng: lng.toString() } });
  };
      ...
      {isLoading ? (
        ...
        <ScrollView contentContainerStyle={{ paddingBottom: insets.bottom + 24 }}>
          ...
          <View className="px-4 pb-2 flex-row gap-2">
            <TouchableOpacity
              onPress={() => handleCall(tallerData.taller.telefono)}
              className="flex-1 border border-navy-600 py-2.5 rounded-xl flex-row items-center justify-center gap-2"
              activeOpacity={0.8}
            >
            ...
            <TouchableOpacity
              onPress={() => handleRuta(tallerData.taller.lat, tallerData.taller.lng)}
              className="flex-1 border border-navy-600 py-2.5 rounded-xl flex-row items-center justify-center gap-2"
              activeOpacity={0.8}
            >
          ...
          {tallerData.especialidades.length > 0 && (
            <>
              ...
              <View className="px-4">
                <ChipList
                  chips={tallerData.especialidades.map((e) => ({ id: e.id_especialidad, label: e.nombre_especialidad }))}
                  selected={tallerData.especialidades.map((e) => e.id_especialidad)}
                  readonly
                />
              </View>
            </>
          ...
          {tallerData.servicios.length > 0 && (
            <>
              <SectionLabel
                label="Servicios"
                action={tallerData.servicios.length > 5 ? `Ver más (${tallerData.servicios.length}) →` : undefined}
                onAction={() => router.push(`/shop/services-list?id=${id}` as any)}
              />
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 4 }}>
                {tallerData.servicios.slice(0, 5).map((s) => (
          ...
          {conductorId && myRating == null && (
            <View className="px-4 mt-3">
              <TouchableOpacity
                ...
                className="bg-accent py-3 rounded-xl flex-row items-center justify-center gap-2"
                activeOpacity={0.85}
              >
        ...
        <ScrollView contentContainerStyle={{ paddingBottom: insets.bottom + 24 }}>
          ...
          <View className="px-4 pb-2 flex-row gap-2">
            <TouchableOpacity
              onPress={() => handleCall(proveedorData.proveedor.telefono)}
              className="flex-1 border border-navy-600 py-2.5 rounded-xl flex-row items-center justify-center gap-2"
              activeOpacity={0.8}
            >
              <Icon name="call" size={16} color="#FFFFFF" />
            ...
            <TouchableOpacity
              onPress={() => handleRuta(proveedorData.proveedor.lat, proveedorData.proveedor.lng)}
              className="flex-1 border border-navy-600 py-2.5 rounded-xl flex-row items-center justify-center gap-2"
              activeOpacity={0.8}
            >
              <Icon name="route" size={16} color="#FFFFFF" />
          ...
          {proveedorData.productos.length > 0 && (
            <>
              <SectionLabel
                label="Productos"
                action={proveedorData.productos.length > 5 ? `Ver más (${proveedorData.productos.length}) →` : undefined}
                onAction={() => router.push(`/shop/products-list?id=${id}` as any)}
              />
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 4 }}>
                {proveedorData.productos.slice(0, 5).map((p) => (
                  <ProductoCard
                    ...
                    nombre={p.nombre}
                    precio={p.precio}
                    imagenUrl={p.imagenes[0]?.imagen_url}
                  />
                ))}
              </ScrollView>
            </>
